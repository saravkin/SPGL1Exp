function [x] = TraceNorm_projectweight1(x,weights,B,params)
...
fr = trace(v*diag([(w^2)*(lam*w^2 + 1)^-2*ones(q,1); (lam+1)^-2*ones(m-q,1)])*v'*(R*R'));
f = 0.5*(fl+fr)-tau;
i=0;
while abs(f) >= 1e-6 
    i = i+1;
    ...
    gr = trace(v*diag([(-2*w^4)*(lam*w^2 + 1)^-3*ones(q,1); -2*(lam+1)^-3*ones(m-q,1)])*v'*(R*R'));
    g=(gl+gr);
    lam = lam - f/g;
    [i lam f g];
end
