% addpath(genpath('/Dropbox/spgl1Latest'));
% addpath(genpath('/Dropbox/MaxNorm/GulfofSuez350/code/Function'));
clear all;
addpath(genpath('/users/slic/rkumar/spgl1Latest'));
%% fetch the data from cluster
RandStream.setDefaultStream(RandStream('mt19937ar','seed',1));
nt = 1024;
nr = 355;
ns = 355;
D=ReadSuFast('/scratch/slic/rkumar/suezdatatest/SuezShots125-355shots.su');
D = reshape(D,nt,nr,ns);  
data = fft(D);
clear D
data = permute(reshape(data,nt,nr,ns),[3 2 1]);
freq = 0; % for low frequency else high frequency
a=250;
D = squeeze((data(1:352,1:352,a)));
nm = size(D);
nR = nm(2);
nC = nm(1);% length of frequency axis
opSR=opSR2MH(nR/2);
info=opSR([],0);
SR=opFunction(info{1},info{2},opSR);
%% Define Restriction Operator
inds=randperm(nR);
ind=inds(1:floor(nR/2));
R1 = opRestriction(nR,ind);
R2 = opKron(R1,opDirac(nR));
Bfun = @(x)R2'*R2*vec(x);
b = Bfun(D);
b=reshape(b,nR,nC);
params.nr = 20;
%% Partition of a matrix in SR domain
%
%
%       |
%    A  |   B
%  -----------
%    C  |   D
%       |
%
%% SPGL1 for A
A = b(1:nR/2,1:nC/2);
A=SR*vec(A);
A=reshape(A,nR/2,nC);
params.afunT = @(x)reshape(x,size(A,1),size(A,2));
params.numr = size(A,1);
params.numc = size(A,2);
A=vec(A);
params.Ind = find(A==0);
params.afun = @(x)afun(x,params.Ind);
params.funForward = @NLfunForward;
% options for trace norm
opts = spgSetParms('optTol',1e-5, ...
                   'bpTol', 1e-5,...
                    'decTol',1e-4,...
                   'project', @TraceNorm_project, ...
                   'primal_norm', @TraceNorm_primal, ...
                   'dual_norm', @TraceNorm_dual, ...
                   'proxy', 1, ...
                   'ignorePErr', 1, ...
                   'iterations', 150,...
                   'weights', []);
LInit   = randn(params.numr,params.nr)+1i*randn(params.numr,params.nr);
RInit   = randn(params.numc,params.nr)+1i*randn(params.numc,params.nr);
xinit  = [vec(LInit);vec(RInit)]; % Initial guess
tau = norm(xinit,1);
sigmafact=0.001;
sigma=sigmafact*norm(A,2);
opts.funPenalty = @funLS;
tic;
[xLRA,r,g,info] = spgl1(@NLfunForward,A,tau,sigma,xinit,opts,params);
toc;
e = params.numr*params.nr;
L1 = xLRA(1:e);
R1 = xLRA(e+1:end);
L1 = reshape(L1,params.numr,params.nr);
R1 = reshape(R1,params.numc,params.nr);
xlra = SR'*vec(L1*R1');
xlra=reshape(xlra,nR/2,nC/2);
%% SPGL1 for B
B = b(nR/2+1:end,nC/2+1:end);
B=SR*vec(B);
B=reshape(B,nR/2,nC);
params.afunT = @(x)reshape(x,size(B,1),size(B,2));
params.numr = size(B,1);
params.numc = size(B,2);
B=vec(B);
params.Ind = find(B==0);
params.afun = @(x)afun(x,params.Ind);
params.funForward = @NLfunForward;
% options for trace norm
opts = spgSetParms('optTol',1e-5, ...
                   'bpTol', 1e-5,...
                    'decTol',1e-4,...
                   'project', @TraceNorm_project, ...
                   'primal_norm', @TraceNorm_primal, ...
                   'dual_norm', @TraceNorm_dual, ...
                   'proxy', 1, ...
                   'ignorePErr', 1, ...
                   'iterations', 150,...
                   'weights', []);
LInit   = randn(params.numr,params.nr)+1i*randn(params.numr,params.nr);
RInit   = randn(params.numc,params.nr)+1i*randn(params.numc,params.nr);
xinit  = [vec(LInit);vec(RInit)]; % Initial guess
tau = norm(xinit,1);
sigmafact=0.001;
sigma=sigmafact*norm(B,2);
opts.funPenalty = @funLS;
tic;
[xLRA,r,g,info] = spgl1(@NLfunForward,B,tau,sigma,xinit,opts,params);
toc;
e = params.numr*params.nr;
L1 = xLRA(1:e);
R1 = xLRA(e+1:end);
L1 = reshape(L1,params.numr,params.nr);
R1 = reshape(R1,params.numc,params.nr);
xlrb = SR'*vec(L1*R1');
xlrb=reshape(xlrb,nR/2,nC/2);
%%
B = b(1:nR/2,nC/2+1:end);
B=SR*vec(B);
B=reshape(B,nR/2,nC);
params.afunT = @(x)reshape(x,size(B,1),size(B,2));
params.numr = size(B,1);
params.numc = size(B,2);
B=vec(B);
params.Ind = find(B==0);
params.afun = @(x)afun(x,params.Ind);
params.funForward = @NLfunForward;
% options for trace norm
opts = spgSetParms('optTol',1e-5, ...
                   'bpTol', 1e-5,...
                    'decTol',1e-4,...
                   'project', @TraceNorm_project, ...
                   'primal_norm', @TraceNorm_primal, ...
                   'dual_norm', @TraceNorm_dual, ...
                   'proxy', 1, ...
                   'ignorePErr', 1, ...
                   'iterations', 150,...
                   'weights', []);
LInit   = randn(params.numr,params.nr)+1i*randn(params.numr,params.nr);
RInit   = randn(params.numc,params.nr)+1i*randn(params.numc,params.nr);
xinit  = [vec(LInit);vec(RInit)]; % Initial guess
tau = norm(xinit,1);
sigmafact=0.001;
sigma=sigmafact*norm(B,2);
opts.funPenalty = @funLS;
tic;
[xLRA,r,g,info] = spgl1(@NLfunForward,B,tau,sigma,xinit,opts,params);
toc;
e = params.numr*params.nr;
L1 = xLRA(1:e);
R1 = xLRA(e+1:end);
L1 = reshape(L1,params.numr,params.nr);
R1 = reshape(R1,params.numc,params.nr);
xlrc = SR'*vec(L1*R1');
xlrc=reshape(xlrc,nR/2,nC/2);
%%
B = b(nR/2+1:end,1:nC/2);
B=SR*vec(B);
B=reshape(B,nR/2,nC);
params.afunT = @(x)reshape(x,size(B,1),size(B,2));
params.numr = size(B,1);
params.numc = size(B,2);
B=vec(B);
params.Ind = find(B==0);
params.afun = @(x)afun(x,params.Ind);
params.funForward = @NLfunForward;
% options for trace norm
opts = spgSetParms('optTol',1e-5, ...
                   'bpTol', 1e-5,...
                    'decTol',1e-4,...
                   'project', @TraceNorm_project, ...
                   'primal_norm', @TraceNorm_primal, ...
                   'dual_norm', @TraceNorm_dual, ...
                   'proxy', 1, ...
                   'ignorePErr', 1, ...
                   'iterations', 150,...
                   'weights', []);
% Run the main algorithm
LInit   = randn(params.numr,params.nr)+1i*randn(params.numr,params.nr);
RInit   = randn(params.numc,params.nr)+1i*randn(params.numc,params.nr);
xinit  = [vec(LInit);vec(RInit)]; % Initial guess
tau = norm(xinit,1);
sigmafact=0.001;
sigma=sigmafact*norm(B,2);
opts.funPenalty = @funLS;
tic;
[xLRA,r,g,info] = spgl1(@NLfunForward,B,tau,sigma,xinit,opts,params);
toc;
e = params.numr*params.nr;
L1 = xLRA(1:e);
R1 = xLRA(e+1:end);
L1 = reshape(L1,params.numr,params.nr);
R1 = reshape(R1,params.numc,params.nr);
xlrd = SR'*vec(L1*R1');
xlrd=reshape(xlrd,nR/2,nC/2);
Xrec=[xlra xlrc;xlrd xlrb];
SNR1 = -20*log10(norm(D-Xrec,'fro')/norm(D,'fro'))

